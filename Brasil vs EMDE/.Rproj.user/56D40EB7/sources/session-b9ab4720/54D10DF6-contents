...
# ---- 5) Métricas (Peru vs EMDE ex-China ex-Peru)

metricas_peru <- base_final %>%
  summarise(
    anos_validos = n(),
    pct_abaixo_45 = 100 * mean(crescimento_pais < crescimento_emde_sem_china_e_pais, na.rm = TRUE),
    
    crescimento_mediano_peru = median(crescimento_pais, na.rm = TRUE),
    crescimento_mediano_emde_benchmark = median(crescimento_emde_sem_china_e_pais, na.rm = TRUE),
    
    pct_crescimento_negativo_peru = 100 * mean(crescimento_pais < 0, na.rm = TRUE),
...
grafico <- ggplot(base_final, aes(x = crescimento_emde_sem_china_e_pais,
  ...
  scale_y_continuous(expand = expansion(mult = 0.05),
                     labels = function(x) paste0(x, "%")) +
  labs(
    title = "Peru vs Emergentes e Em Desenvolvimento (EMDE)",
    subtitle = "Crescimento real do Peru versus grupo dos países emergentes e em desenvolvimento de 1980 a 2024",
    x = "Países Emergentes e Em Desenvolvimento (exceto China e Peru)",
    y = "Peru"
  ) +
  theme_classic() +
  theme(
...
# ---- 5) Gráfico alternativo: regressão (Peru vs EMDE ex-China ex-Peru)
...
grafico_regressao <- ggplot(base_final,
  ...
  labs(
    title = "Peru vs EMDE (exceto China e Peru): reta de regressão (1980–2024)",
    x = "EMDE (exceto China e Peru) — crescimento real (%)",
    y = "Peru — crescimento real (%)"
  ) +
  theme_classic() +
  theme(
    plot.title = element_text(face = "bold"),
